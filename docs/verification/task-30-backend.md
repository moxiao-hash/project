...
- **执行时间**：2026-09-11（首轮交付 11:06；Codex 复审整改 11:40，Asia/Shanghai）
...
- **提交**：
  ...
  - 复审整改：`fix: enforce strict tool output contracts and runtime timeouts`（见 `git log -1`，未改写 `9f747f2`）
...
| 4 | `timeoutMillis` 仅元数据，未在运行时强制 | 新增 `AgentToolTimeoutGuard` + `AgentToolTimeoutException`：读工具在注册表边界、写工具在治理执行边界按 `descriptor.timeoutMillis()` 强制；超时映射 504（读）/ FAILED 动作（写）；新增慢 Handler 回归测试 |
...
- Python `test_supervisor_receipts.py`（13）：动作归属/幂等/409/失败不成功/有界重试/人工入口、
...
PYTHONPATH=$PWD .venv/bin/python -m pytest -q                        # 419 passed, 1 warning
PYTHONPATH=$PWD .venv/bin/python -m ruff check app tests             # All checks passed!

cd ../backend
./mvnw -o test                                                       # 405 passed, 0 failures, 0 errors
                                                                     # BUILD SUCCESS（Task 30 首轮基线 374，复审前 396）
...
- `AgentToolTimeoutGuard`：守护线程池执行 + `Future.get(timeout)`；超时 `cancel(true)` 并抛
  `AgentToolTimeoutException`；业务异常原样透传。
- 读工具在 `AgentToolRegistry` 边界受控；写工具在 `AgentToolActionService.executeClaim` 通过
  `AgentToolTimeoutGuard` 包裹 `AgentToolBusinessExecutor.execute`（`REQUIRES_NEW` 事务在被调度线程上开启），
  超时被治理层捕获并落为 `FAILED` 动作（含脱敏错误 + 通知 + 审计），绝不视为成功。
- `GlobalExceptionHandler` 将读链路的 `AgentToolTimeoutException` 映射为 **504**。
- 已知限制：JDBC/阻塞 IO 不保证响应线程中断；护栏保证调用方在超时点确定性失败，底层调用返回后其事务随线程结束。
...
**Java 生产**
- `agent/tool/AgentToolOutputSchemas.java`（重写为递归严格建模）
- `agent/tool/AgentToolOutputValidator.java`（受控映射值校验）
- `agent/tool/AgentToolRegistry.java`（裁剪信封 + 读超时）
- `agent/tool/AgentToolActionService.java`（写执行超时）
- `agent/tool/AgentToolTimeoutGuard.java`（新增）
- `agent/tool/AgentToolTimeoutException.java`（新增）
- `shared/api/GlobalExceptionHandler.java`（504 映射）

**Java 测试**
- 新增 `AgentToolOutputSchemasTest`、`AgentToolTimeoutGuardTest`
- 修改 `AgentToolRegistryTest`、`AgentToolCoverageTest`

**Python**
- `app/unified_agent/supervisor.py`（重试持久化顺序）
- `tests/unified_agent/test_supervisor_receipts.py`（+3 持久化顺序测试）
...
- 复审整改为**新提交**（未 amend `9f747f2`），只提交自有文件，只推送 `agent/deepseek-task-30-tools`。
