...
>
> 契约基线：阶段 7 完成版本（2026-07-28，数据库迁移 V1～V20）
>
> Java 地址：`http://localhost:8080`
>
> Python 地址：`http://localhost:8000`

本文是前端开发的主要上下文。文中接口分成两类：

- **✅ 当前可用**：Java 已经公开的 `/api/**`，前端可以直接联调。
- **🟨 规划契约**：阶段 8 将由 Java 增加的 Agent 门面。前端可以先完成页面、类型和
  Mock Adapter，但目前不能向真实地址发送请求。
...
StudyPilot 是个人 AI 学习执行工作台。用户可以建立目标、生成计划、完成每日任务、
导入资料、进行带来源的知识问答、完成自适应测验，并让 Agent 在明确授权和确认边界
内调整学习安排。
...
- `planned` 下定义统一接口，并提供 `MockAgentGateway` 与未来的
  `HttpAgentGateway`；切换实现不能影响页面。
...
| `/agent/plan` | 对话生成计划 | 🟨 等待 Java 门面 |
| `/agent/tasks` | 对话操作任务 | 🟨 等待 Java 门面 |
| `/knowledge` | RAG 知识问答 | 🟨 等待 Java 门面 |
| `/settings/ai` | 模型与搜索 Key | 🟨 等待安全存储 |
...
## 7. 阶段 8 规划中的 Java Agent 门面

> **🟨 以下接口当前尚未实现。**
>
> Kimi 可以定义类型、页面和 Mock 数据，但 `HttpAgentGateway` 默认关闭。后续 Java
> 门面负责从 Bearer Token 取得用户 ID，注入 `ownerId` 后调用 Python。
...
提交成功后清空输入和组件状态。上述安全存储尚未实现，现在仍由 AI 服务 `.env` 提供
服务器级 Key。
