...
Task 11 已实现成果登记，但安全 Runner 和 AI 验收尚未实现。浏览器可以登记目录和提交
已有成果，不能通过这些接口执行命令。
...
响应状态当前为 `SUBMITTED`，并返回 `submissionVersion`、`evaluationMode`、规范路径、
...
或成果不属于当前用户时返回 404；不要求成果的节点或状态冲突返回 409。浏览器路径选择
能力有限，当前页面可使用显式路径输入并解释这是本机路径；Task 12～14 完成后再接入
执行预览、确认、Runner 结果与 AI Rubric 验收 UI。
