...
```text
POST   /api/assistant/automation-rules
GET    /api/assistant/automation-rules
GET    /api/assistant/automation-rules/{ruleId}
PATCH  /api/assistant/automation-rules/{ruleId}
DELETE /api/assistant/automation-rules/{ruleId}
GET    /api/assistant/automation-settings
PATCH  /api/assistant/automation-settings
```
...
创建请求为 `{ type, timezone, localTime, enabled }`，修改请求可只传
`enabled / timezone / localTime` 中需要变化的字段。全局暂停使用
`PATCH /api/assistant/automation-settings` 并传 `{ "paused": true }`。

规则本身不等于授权。Java 每次执行前仍会检查长期授权；高风险操作始终只发送待确认
通知。规则创建、修改、暂停和删除必须由用户在专用设置界面点击完成，不允许普通聊天
代替用户管理授权范围。

### 7.8 Agent 运行健康（Task 20 首批）

`GET /api/assistant/health` 要求 Bearer 登录，无需请求参数。返回当前用户的
`totalExecutions / successfulExecutions / failedExecutions / successRate / promptTokens /
completionTokens / estimatedCost / averageLatencyMs / pendingConfirmations`，并返回
`costSamples / tokenSamples / latencySamples`。无样本必须显示“暂无数据”。
成功率只按成功与失败记录计算，不含待确认和拒绝操作。当前用量只累计已上报执行记录，
不是全部统一对话的成本账单。前端入口为 `/assistant/health`。
...
侧边栏“学习”分组新增 `/wrong-questions`。浏览器仍只调用 Java 公共接口：

```text
GET  /api/wrong-questions/summary
GET  /api/wrong-questions?status=ACTIVE|MASTERED&chapterKey=&page=0&size=20
POST /api/wrong-question-reviews
GET  /api/wrong-question-reviews/current
```

创建重做批次时提交可选 `chapterKey` 和必填 `idempotencyKey`。Java 从当前登录身份注入
用户，每次选择最多 5 道待重做题并返回 `quizId`；前端随后复用现有测验和 attempt
接口。批次未完成时 `/current` 返回可恢复的测验，完成后返回 204。

`QuizResponse.kind` 为 `WRONG_QUESTION_REVIEW` 时，题目来自历史原题，不产生新的出题
或搜索额度。`QuizAttemptResponse` 在提交后增加题干、用户答案、正确答案/参考实现；
错题批次还包含 `reviewProgress.clearedCount` 和 `remainingCount`。这些答案字段只能出现在
已提交的 attempt 或错题详情中，普通 Quiz 读取接口仍不返回正确答案。

---

## 12. 路线工作区与实践成果接口

Task 22～23 已实现安全 Runner 和 AI Rubric 验收。浏览器只调用 Java，由 Java 通过
Unix Socket 调用 Runner，并在用户确认源码清单后通过 FastAPI 请求 DeepSeek。

```text
POST /api/workspaces
GET  /api/workspaces

POST /api/roadmap-artifacts
GET  /api/roadmap-artifacts/{artifactId}
POST /api/roadmap-artifacts/{artifactId}/review-previews
POST /api/roadmap-artifacts/{artifactId}/review-previews/{previewId}/confirm
POST /api/roadmap-artifacts/{artifactId}/accept
POST /api/roadmap-artifacts/{artifactId}/reject

POST /api/runner/preview
POST /api/runner/executions
GET  /api/runner/executions/{runnerExecutionId}
POST /api/runner/executions/{runnerExecutionId}/confirm
POST /api/runner/executions/{runnerExecutionId}/reject
```

工作区创建请求：

```ts
interface CreateProjectWorkspaceRequest {
  name: string
  rootPath: string // 本机已存在的绝对目录；最长 1024 字符
}
```

工作区响应包含 `id`、`name`、规范化后的 `rootPath`、`status: 'ACTIVE'` 和
`createdAt`。工作区由当前登录用户隔离，不能提交 `ownerId`。

成果提交请求：

```ts
interface CreateRoadmapArtifactRequest {
  workspaceId: string
  roadmapNodeId: string
  relativePath: string // 相对于工作区的已存在路径
  description: string
  testEvidence: string
  idempotencyKey: string
}
...
成果提交后状态为 `SUBMITTED`，并返回 `submissionVersion`、`evaluationMode`、规范路径、
阶段/模块/节点不可变快照和 `reviewHistory`。`SUBMITTED` 仅代表等待验收，前端不能显示
“已通过”或“节点已完成”。相同幂等键重试时必须复用完全相同的请求内容。

AI 评审前必须先取得同工作区、晚于成果提交的成功 Runner 测试记录。创建评审预览时
提交 `runnerExecutionId` 和稳定 `idempotencyKey`，响应会列出待发送文件、排除路径、
总字节数和十分钟过期时间。只有专用确认接口会发送清单内源码；确认前发生文件变化时
返回 409。AI 达到 70 分也不会自动验收，用户仍需调用 `accept`。

错误处理：相对工作区根、目录不存在、`..` 越界和符号链接路径返回 400；工作区、路线、
Runner 记录或成果不属于当前用户时返回 404；测试证据无效、文件变化、预览过期及状态
冲突返回 409；AI 服务或用户凭据不可用返回 503。前端后续应把 Runner 执行、源码清单
确认、Rubric 结果和最终人工接受设计为四个明确步骤，不能合并确认。

---

## 13. Developer Agent 受控源码接口

Task 24 已提供源码读取和补丁治理基础。公共接口仍只接受 Bearer Token，并按当前登录用户
校验工作区归属：

```text
GET  /api/developer/workspaces/{workspaceId}/tree
GET  /api/developer/workspaces/{workspaceId}/file?path=
GET  /api/developer/workspaces/{workspaceId}/search?q=
GET  /api/developer/workspaces/{workspaceId}/git-status
GET  /api/developer/workspaces/{workspaceId}/git-diff
GET  /api/developer/workspaces/{workspaceId}/git-log?limit=20
POST /api/developer/workspaces/{workspaceId}/patch-preview?targetFile=
```

`patch-preview` 的请求体是 `text/plain` Unified Diff。成功预览返回 `expectedSha256`、
`resultSha256`、`affectedLines` 和 `safeToApply`。真正应用补丁不是公共直写接口，而是
Java 工具目录中的 `developer.patch.apply` 高风险操作；前端只展示统一 Agent 返回的操作
卡片，并通过既有 `/api/assistant/conversations/{conversationId}/actions/{actionId}/confirm`
专用接口确认。

读取和搜索均限制为登记工作区内的白名单文本文件；`.env`、应用配置、证书、构建输出、
二进制文件和符号链接会被拒绝，普通源码或 Git 输出中的常见凭据会脱敏。补丁只支持单个
既有文本文件，最大 20,000 字符。预览后文件发生变化时，确认执行会失败并要求重新预览。
Task 25 已增加以下公共预览接口：

```text
POST /api/developer/workspaces/{workspaceId}/test-recommendations
POST /api/developer/workspaces/{workspaceId}/git-commit-preview
GET  /api/developer/workspaces/{workspaceId}/git-push-preview
```

真正执行继续由统一 Agent 工具 `runner.check.run`、`developer.git.commit` 和
`developer.git.push` 发起。commit 与 push 固定为两张互相独立的高风险确认卡：确认 commit
只创建本地提交，不会顺带推送；push 必须重新核对 `origin`、当前分支和 HEAD 后再次确认。

测试推荐只会返回 Runner 已注册的 Maven、npm 或 pytest 模板。界面操作始终优先使用业务
API；没有 API 时也只能从预注册的 Playwright DOM / IDEA Accessibility 动作中选择，前端和
模型均不能提供任意 URL、CSS selector、脚本或键鼠序列。
