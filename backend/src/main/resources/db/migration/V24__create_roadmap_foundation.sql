...
CREATE TABLE roadmap_nodes (
    ...
    CONSTRAINT fk_roadmap_nodes_stage FOREIGN KEY (stage_id) REFERENCES roadmap_stages (id),
...
CREATE TABLE roadmap_node_prerequisites (
    ...
    CONSTRAINT fk_roadmap_prerequisites_node FOREIGN KEY (node_id) REFERENCES roadmap_nodes (id),
    CONSTRAINT fk_roadmap_prerequisites_required_node FOREIGN KEY (prerequisite_node_id) REFERENCES roadmap_nodes (id),
...
CREATE TABLE user_roadmap_nodes (
    ...
    updated_at TIMESTAMP(6) NOT NULL,
    row_version BIGINT NOT NULL DEFAULT 0,
    CONSTRAINT fk_user_roadmap_nodes_roadmap FOREIGN KEY (user_roadmap_id) REFERENCES user_roadmaps (id),
    CONSTRAINT fk_user_roadmap_nodes_node FOREIGN KEY (node_id) REFERENCES roadmap_nodes (id),
...
CREATE TABLE roadmap_upgrades (
    ...
    CONSTRAINT fk_roadmap_upgrades_user_roadmap FOREIGN KEY (user_roadmap_id) REFERENCES user_roadmaps (id),
...
CREATE TABLE legacy_lesson_roadmap_mappings (
    ...
    CONSTRAINT fk_legacy_mapping_node FOREIGN KEY (node_id) REFERENCES roadmap_nodes (id)
);
...
CREATE TABLE legacy_learning_evidence (
    ...
    CONSTRAINT fk_legacy_evidence_user_node FOREIGN KEY (user_roadmap_node_id) REFERENCES user_roadmap_nodes (id),
...
CREATE INDEX idx_roadmap_stages_template ON roadmap_stages (template_id);
CREATE INDEX idx_roadmap_nodes_stage ON roadmap_nodes (stage_id);
CREATE INDEX idx_roadmap_prerequisites_node ON roadmap_node_prerequisites (node_id);
